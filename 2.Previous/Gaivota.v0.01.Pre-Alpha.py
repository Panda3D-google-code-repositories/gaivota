...
class MainMenu(DirectObject.DirectObject):
    ...
    def __init__(self):
        ...
        self.quitButton = DirectButton(parent=self.frame, text="quit game", command=sys.exit, pos=(0,0,-0.1), text_scale=0.06, borderWidth=(0.005,0.005), frameSize=(-0.25, 0.25, -0.03, 0.06))
        self.backButton = DirectButton(parent=self.frame, text="back to game", command=self.hideMenu, pos=(0,0,-0.3), text_scale=0.06, borderWidth=(0.005,0.005), frameSize=(-0.25, 0.25, -0.03, 0.06))
...
class Game(DirectObject.DirectObject):
    def __init__(self):
        ...
        self.turrets = NodePath("turrets")
        self.turrets.reparentTo(render)
        self.turrets.flattenStrong()
               
        self.accept('tower-down', self.evtTowerDown)
        self.accept('player-death', self.evtPlayerDeath)
        self.accept('mouse1',self.evtShoot)
        self.spawn()
        
        '''
        shader = loader.loadShader("radial_blur.sha")
    ...
    def evtPlayerDeath(self):
        ...
        self.msg.addMessage("You made it to level %i" %( self.lvl ), 600)
        self.msg.addMessage("You destroyed %i towers" %( self.kills ), 600)
        self.msg.addMessage("You shoot %i missiles" %( self.shoots ), 600)
        if self.shoots > 0:
            self.msg.addMessage("That's an accuracy of %i percent" %( round( ((4.0*self.kills)/self.shoots*100))), 600)
    ...
    '''
    evtTowerDown
    each time a tower is destroyed, we reduce the number of active towers,
    if the number reaches 0, we spawn new ones
    '''        
    def evtTowerDown(self):
        self.msg.addMessage("tower destroyed", 4)
        self.towers -= 1
        self.kills +=1
        if self.towers == 0:
            self.lvl +=1
    ...
    '''
    spawn
    Spawns a number of new towers depending on the current level.
    Level = towers to spawn
    '''
    def spawn(self):
        self.towers = self.lvl
        self.msg.addMessage("lvl %i spawning %i tower(s)" %( self.lvl, self.towers ), 6)
        loc = 500
        for i in range(0,self.lvl):
            xrand = 0
            yrand = 0
            while  ((-170 < xrand < 170 ) or (-170 < yrand < 170 )):
                xrand = random.randrange(-loc, loc, 25)
                yrand = random.randrange(-loc, loc, 25)
            t = Tower([xrand,yrand,random.randrange(-200, 200, 60)])
            t.node.reparentTo(self.turrets);
########################################################################################################
...
class Credits(DirectObject.DirectObject):
    ...
    def __init__(self):
        ...
        self.Text = DirectLabel(
                                      parent=self.frame, 
                                      text="Programming and models by Akuryou\n\nakuryou@l2si.de\n\nplane model by Nemesis#13", 
                                      scale=0.07, 
                                      frameColor=(0,0,0,0), 
                                      pos=(-0.48,0,0.1),
                                      text_align=TextNode.ALeft)
